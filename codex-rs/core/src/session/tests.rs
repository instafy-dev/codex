...
async fn refresh_mcp_servers_is_deferred_until_next_turn() {
    ...
    session
        ...
        .await
        .expect("empty MCP refresh should shut down the previous manager");
    ...
    let lifecycle = session
        .services
        .mcp_connection_manager_lifecycle
        .lock()
        .await;
    assert!(!lifecycle.refresh_in_progress);
    assert!(!lifecycle.refresh_tainted);
    assert!(lifecycle.retired.is_empty());
}

#[tokio::test]
async fn interrupted_mcp_refresh_prevents_confirmed_session_shutdown() {
    let (session, _turn_context) = make_session_and_context().await;
    {
        let mut lifecycle = session
            ...
            .lock()
            .await;
        // This is the state an aborted refresh leaves behind when cancellation happens after
        // manager construction may have launched stdio but before registration/confirmation.
        lifecycle.refresh_in_progress = true;
    }

    let error = session
        .services
        .shutdown_mcp_connection_managers_confirmed()
        .await
        .expect_err("an interrupted refresh must fail the ShutdownComplete barrier");
    assert!(
        error.to_string().contains("refresh remains in progress"),
        "unexpected shutdown error: {error:#}"
    );
}

#[tokio::test]
async fn successful_mcp_refresh_does_not_clear_an_earlier_interruption() {
    let (session, turn_context) = make_session_and_context().await;
    {
        let mut lifecycle = session
            .services
    ...
    session
        ...
        .await
        .expect("the later empty refresh itself should complete");
    ...
    let error = session
        ...
        .shutdown_mcp_connection_managers_confirmed()
        .await
        .expect_err("later refresh success must not erase earlier lifecycle uncertainty");
    assert!(
        error.to_string().contains("earlier MCP refresh"),
        "unexpected shutdown error: {error:#}"
    );
}
